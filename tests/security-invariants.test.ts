...
const { invokeHttpBackend, ToolBackendError } = await import("../src/backends/http.ts");
...
const policyPath = fileURLToPath(new URL("../tool_risk_tier.yaml", import.meta.url));
const policy = loadPolicy(policyPath);

function getPostHandlers(router: { stack: Array<{ route?: { path: string; methods: Record<string, boolean>; stack: Array<{ handle: ExpressHandler }> } }> }, path: string): ExpressHandler[] {
...
async function runHandlers(handlers: ExpressHandler[], req: MockRequest): Promise<MockResponse> {
...
test("invoke-tool rejects request-controlled policy_set_id", async () => {
  const router = createInvokeRouter(() => policy, new QuotaTracker(), async () => ({ ok: true }));
  const handlers = getPostHandlers(router as unknown as { stack: Array<{ route?: { path: string; methods: Record<string, boolean>; stack: Array<{ handle: ExpressHandler }> } }> }, "/");
...
test("invoke-tool sanitizes backend failures before returning them", async (t) => {
  ...
  assert.equal((res.body as { error: string }).error, "Tool backend error for search_documents");
...
test("policy reload requires an administrative session", async () => {
  ...
  const handlers = getPostHandlers(router as unknown as { stack: Array<{ route?: { path: string; methods: Record<string, boolean>; stack: Array<{ handle: ExpressHandler }> } }> }, "/reload");
