...
export function findToolOverride(policy: PolicyDoc, toolName: string): ToolOverride | null {
...
export function resolveEffectiveTier(
  ...
  if (override?.force_tier) {
    ...
    if (!forced) throw new Error(`Override references unknown tier: ${forcedId}`);
