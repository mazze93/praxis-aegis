...
import { type Caller, callerVerificationOrThrow } from "../enforce/caller.js";
import {
	applyQuotasOrThrow,
	type QuotaTracker,
	trackResponseBytes,
} from "../enforce/quota.js";
...
function sanitizeErrorForCaller(
	err: unknown,
	tier: TierDef | undefined,
): string {
...
export function createInvokeRouter(
	...
	router.post("/", requireSession, async (req: Request, res: Response) => {
		...
		try {
			...
			const baseTierId = resolveTierId(
				policy,
				policySetId,
				inv.meta.identity,
				inv.meta.trigger,
			);
			const { tierId, tier } = resolveEffectiveTier(
				policy,
				baseTierId,
				inv.tool_name,
			);
			...
			try {
				...
				tierForResponse = resolveEffectiveTier(
					policy,
					baseTierId,
					inv.tool_name,
				).tier;
