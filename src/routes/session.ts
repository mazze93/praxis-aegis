...
import { createChallenge, consumeChallenge, issueSessionToken } from "../auth/session.js";
...
sessionRouter.post("/challenge", rateLimiter, (req: Request, res: Response) => {
  ...
  if (!fs.existsSync(allowedSignersPath)) {
    res.status(400).json({ ok: false, error: `Unknown identity: "${identity}"` });
    return;
  }
 ...
 */
sessionRouter.post("/unlock", rateLimiter, async (req: Request, res: Response) => {
