...
function requireAdminIdentity(req: Request, res: Response): boolean {
  ...
  if (config.ADMIN_IDENTITIES.length === 0) {
    res.status(403).json({ ok: false, error: "Policy reload is disabled: no admin identities configured" });
    return false;
  }

  if (!config.ADMIN_IDENTITIES.includes(identity)) {
    res.status(403).json({ ok: false, error: "Administrative session required for policy reload" });
    return false;
  }
...
export function createPolicyRouter(
  ...
  router.get("/sets", requireSession, (_req: Request, res: Response) => {
    ...
    const sets = (policy.policy_sets ?? []).map((p: { id: string; label: string }) => ({
      id: p.id,
      label: p.label,
    }));
